  ...
  const handleExportPDF = async () => {
    ...
    try {
      const canvas = await html2canvas(element, { 
        ...
        scale: 2, // Pour une meilleure qualité sur les écrans retina
        useCORS: true,
        allowTaint: true
      });
      ...
      const pdf = new jsPDF({
        ...
        unit: 'px',
        format: 'a4' // Forcer le format A4
      });
      
      // Ajuster l'image au format A4 (largeur A4 = 445px environ en jsPDF unit: 'px', hauteur = 630px environ, mais dépend de la résolution interne)
      // jsPDF format 'a4' avec unit 'px' à 72dpi: 595.28 x 841.89
      const pdfWidth = pdf.internal.pageSize.getWidth();
      const pdfHeight = (canvas.height * pdfWidth) / canvas.width;
      
      pdf.addImage(imgData, 'PNG', 0, 0, pdfWidth, pdfHeight);
      pdf.save(`MediaKit_${lastNameInput || 'Athlete'}.pdf`);
    } catch (err) {
